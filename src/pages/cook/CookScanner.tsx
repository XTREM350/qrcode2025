    ...
    try {
      ...
      setScanError(
        error instanceof Error && error.message === 'Insufficient balance'
          ? 'Insufficient balance for meal'
          : 'Invalid or unrecognized badge. Please try again.'
      );
      toast.error('Scan failed. ' + (error instanceof Error ? error.message : 'Invalid badge'));
    }
  };
  ...
  const handleConfirm = async () => {
    try {
      ...
      toast.success(`${scannedStudent?.name} marked for ${
        mealOptions.find(opt => opt.value === mealType)?.label || mealType
      }`);
        ...
        <div className="space-y-6">
              ...
              <div className="text-center py-4">
                ...
                <div className="flex justify-center mb-4">
                  <div className="bg-background-300 px-3 py-1 rounded-full text-xs font-medium">
                    Balance: ${scannedStudent.balance.toFixed(2)}
                  </div>
                </div>
          ...
          <div className="border border-gray-700 rounded-lg p-4 bg-background-600 mt-6">
            ...
            <ol className="list-decimal list-inside text-sm text-gray-400 space-y-1">
              <li>Select the meal type from the dropdown</li>
              <li>Click "Start Scanning" and point camera at student's badge</li>
              <li>Verify student details and confirm</li>
              <li>The system will automatically record the meal service</li>
            </ol>
          </div>
        </div>
      </Card>
